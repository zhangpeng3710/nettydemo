...
| 约束 | 说明 |
|------|------|
| **租户与客户端** | ~10,000 租户，每租户 1~2 个客户端实例（近 1:1），共约 10,000~20,000 实例 |
| **版本类型** | 标准版（大部分租户共用）+ 定制版（部分租户独立 JAR） |
...
flowchart LR
    subgraph 研发
        ...
        BUILD --> UPLOAD["上传制品到<br/>Nginx / OSS"]
        BUILD --> JSON["version-info.json<br/>含 hash + SQL 模板"]
        JSON --> SQL["编写投产 SQL"]
    end
    ...
    subgraph 生产
        DBA --> DB[(MySQL)]
        DB --> ENGINE["发布引擎轮询 DB<br/>发现 PENDING 任务"]
        ENGINE --> |"TCP 轻量通知<br/>~100 bytes/客户端"| CLIENT["客户端收到通知"]
        CLIENT --> |"随机延迟 0~N 小时<br/>从 Nginx/OSS 下载"| NGINX["Nginx / OSS<br/>文件服务器"]
    end
```
...
graph TB
    subgraph 标准版流水线
        DEV_S["开发完成"] --> PREVIEW["preview 通道<br/>推送到指定种子租户"]
        PREVIEW --> |"验证通过"| STABLE["stable 通道<br/>全量推送"]
        PREVIEW --> |"发现问题"| FIX["修复后重新 preview"]
    end

    subgraph 定制版
        DEV_C["定制开发"] --> CUSTOM["custom 通道<br/>直接推送到对应租户"]
    end
    ...
    PREVIEW --> T5["种子租户 E 🧪"]
    CUSTOM --> T6["租户 F（定制版）🔧"]
    CUSTOM --> T7["租户 G + H（共享定制版）🔧"]
```

| 通道 | 说明 | 发布范围 |
|------|------|---------|
| `preview` | 预览版，推送到指定种子租户先行验证 | SQL 指定的租户列表 |
| `stable` | 正式版，验证通过后推送给所有标准版租户 | 所有使用标准版的租户 |
| `custom` | 定制版，一家一个或多家共用一个版本 | SQL 指定的租户 |

**灰度流程**就是：先发 preview 到几家种子租户 → 观察无问题 → 再通过投产 SQL 将该版本标记为 stable 推送全量。
...
| 插件差异 | 打标准版 JAR + 独立打插件 JAR | `1.1.0` + 插件独立版本 | 插件放在租户的 `plugin_path` 下 |
| 代码分支 | 切换分支后独立打包 | `1.1.0-CUST_F` | 租户绑定自己的 version 记录 |
...
> [!NOTE]
> 10,000 租户 × 1~2 客户端，近 1:1 关系，不需要分表。所有数据在同一库内。
...
CREATE TABLE tenant (
    ...
    plugin_path     VARCHAR(500)          COMMENT '租户专属插件目录路径（插件差异型定制）',
...
CREATE TABLE tenant_version_publish (
    ...
    min_version     VARCHAR(50)           COMMENT '低于此版本时忽略租户策略，强制更新',
...
> [!IMPORTANT]
> 策略是**租户级别**的配置，存储在 `tenant.update_strategy` 字段中。代表该租户希望的更新行为。通过投产 SQL 设置，也可以由客户端用户在设置界面中选择（上报后更新 DB）。

```mermaid
flowchart TD
    RECEIVE["客户端收到<br/>版本更新通知"] --> CHECK_MIN{"当前版本<br/>< min_version ?"}
    
    CHECK_MIN --> |"是（版本过低）"| FORCE["忽略策略<br/>强制下载 + 自动更新"]
    CHECK_MIN --> |"否"| READ_STRATEGY["读取租户策略<br/>tenant.update_strategy"]
    
    READ_STRATEGY --> NOTIFY["NOTIFY_ONLY<br/>仅提醒"]
    READ_STRATEGY --> AUTO_DL["AUTO_DOWNLOAD<br/>自动下载手动更新"]
    READ_STRATEGY --> FULL["FULL_AUTO<br/>自动下载自动更新"]
    
    NOTIFY --> POPUP1["弹窗提示：<br/>有新版本 X.Y.Z 可用<br/>显示更新说明"]
    POPUP1 --> |"用户点击'下载'"| DL1["开始下载"]
    POPUP1 --> |"用户点击'忽略'"| IGNORE["不做任何操作<br/>记录 SKIPPED"]
    
    AUTO_DL --> DL_BG["后台自动下载<br/>随机延迟后开始"]
    DL_BG --> DONE["下载完成提示：<br/>新版本已就绪<br/>点击'立即更新'"]
    DONE --> |"用户点击'立即更新'"| APPLY["拉起 Launcher<br/>应用更新"]
    DONE --> |"用户点击'稍后'"| WAIT["暂存本地<br/>下次启动时再提示"]
    
    FULL --> DL_BG2["后台自动下载<br/>随机延迟后开始"]
    DL_BG2 --> APPLY2["下载完成后<br/>自动拉起 Launcher 更新"]
    
    FORCE --> DL_FORCE["立即下载（无延迟）"]
    DL_FORCE --> APPLY3["自动更新"]
    
    DL1 --> VERIFY["SHA-256 校验"]
    VERIFY --> APPLY
```
...
> [!NOTE]
> 特殊情况：当客户端当前版本 < 发布任务中的 `min_version` 时，无论租户选择了什么策略，都**强制下载 + 自动更新**。用于处理不兼容的协议变更或安全漏洞修复。
...
> [!NOTE]
> 由于生产参数不能随时调整，**不采用百分比灰度**。改用 **preview 通道** 机制：将新版本先推给指定的种子租户验证，通过后再转为 stable 全量推送。
...
**Step 1：注册 preview 版 + 推给种子租户**

```sql
-- === 投产变更单：发布 v1.2.0 preview 到种子租户 ===
...
```sql
-- === 投产变更单：租户 CUST_F 定制版 v1.1.0-CUST_F ===
...
flowchart TD
    ...
    LOCK --> FIND["查找该租户的<br/>在线客户端 Channel"]
    FIND --> VERSION_CHECK{"客户端 current_version<br/>= 目标 version ?"}
    VERSION_CHECK --> |已是最新| SKIP["跳过，不推送"]
    VERSION_CHECK --> |需更新| SEND["构造 UpgradeCommand<br/>含 version + download_url<br/>+ 租户策略 + min_version"]
...
```java
/**
 * 按租户管理 Channel（替代全局 activeChannels）。
 * 10,000 租户各 1~2 客户端，内存占用极小。
 */
public class TenantChannelManager {

    // tenantCode → 该租户的 Channel 集合（1~2个）
...
graph TB
    ...
    subgraph 投产["投产变更"]
        SQL["SQL 脚本<br/>版本注册 / 租户绑定<br/>preview→stable / 回滚"]
        DBA["DBA 执行"]
    end
    ...
    subgraph 生产["生产环境"]
        ...
        TCM["TenantChannelManager<br/>10,000 租户 Channel"]
...
- 全套投产 SQL 模板：注册版本、preview 发布、stable 全量、定制版、回滚、新增租户
...
#### [NEW] Thymeleaf 页面 — 版本/租户/客户端/日志只读看板
...
2. 执行投产 SQL → 服务端 60s 内拾取 → 推送到指定租户
3. preview 推送种子租户 → 转 stable 全量推送
